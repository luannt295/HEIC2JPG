...
const MAX_FILES = 200;
const HEIC_MODULE_URL = "https://cdn.jsdelivr.net/npm/heic-to@1.5.2/+esm";
const ZIP_MODULE_URL = "https://cdn.jsdelivr.net/npm/jszip@3.10.1/+esm";
...
const isHeicFile = (file: File) => /\.(heic|heif)$/i.test(file.name) || /image\/hei[cf]/i.test(file.type);
...
export function HeicConverter() {
  ...
  function addFiles(files: File[]) {
    const valid = files.filter(isHeicFile);
    ...
    const accepted = unique.slice(0, Math.max(0, MAX_FILES - items.length));
    setItems((current) => [...current, ...accepted.map((file) => ({ id: crypto.randomUUID(), file, status: "ready" as const }))]);
    if (files.length !== valid.length) setNotice(`${files.length - valid.length} file không phải HEIC/HEIF đã được bỏ qua.`);
    else if (accepted.length < unique.length) setNotice(`Mỗi lượt tối đa ${MAX_FILES} ảnh.`);
  ...
  function onDrop(event: DragEvent<HTMLLabelElement>) {
    ...
    setDragging(false);
    if (!converting) addFiles(Array.from(event.dataTransfer.files));
  }
  ...
  async function convert() {
    ...
    setConverting(true);
    setNotice(undefined);
    const { heicTo, isHeic } = await import(/* @vite-ignore */ HEIC_MODULE_URL) as typeof import("heic-to");
    for (const target of targets) {
      setItems((current) => current.map((item) => item.id === target.id ? { ...item, status: "converting", error: undefined } : item));
      try {
        if (!(await isHeic(target.file))) throw new Error("Invalid HEIC file");
  ...
  async function downloadAll() {
    ...
    setZipping(true);
    try {
      const { default: JSZip } = await import(/* @vite-ignore */ ZIP_MODULE_URL) as typeof import("jszip");
      const zip = new JSZip();
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-6xl flex-col px-4 py-5 sm:px-6 lg:px-8">
        ...
        <section className="mx-auto w-full max-w-3xl flex-1 py-12 sm:py-16">
          <div className="mb-8 text-center">
            ...
            <p className="mx-auto mt-4 max-w-xl text-pretty text-sm leading-6 text-[#60756e] sm:text-base">Thả ảnh từ iPhone vào đây. Ảnh được chuyển ngay trong trình duyệt và không bao giờ rời khỏi thiết bị của bạn.</p>
          </div>

          <div className="rounded-[28px] border border-[#d8e1dc] bg-white/90 p-3 shadow-[0_22px_70px_-28px_rgba(28,65,54,.35)] sm:p-5">
            <input id={inputId} type="file" accept=".heic,.heif,image/heic,image/heif" multiple className="sr-only" onChange={onInput} disabled={converting} />
            <label htmlFor={inputId} onDragEnter={() => setDragging(true)} onDragLeave={() => setDragging(false)} onDragOver={(e) => e.preventDefault()} onDrop={onDrop}
              className={`flex min-h-64 cursor-pointer flex-col items-center justify-center rounded-[20px] border-2 border-dashed px-5 text-center transition ${dragging ? "border-[#de8c35] bg-[#fff8ea]" : "border-[#bfd0c8] bg-[#f5f8f5] hover:border-[#5d8f7d]"} ${converting ? "pointer-events-none opacity-60" : ""}`}>
              <span className="mb-5 grid h-16 w-16 place-items-center rounded-2xl bg-[#dfece5] text-[#286250]"><UploadCloud size={31} /></span>
              <span className="text-lg font-bold text-[#24483e]">Kéo và thả ảnh HEIC vào đây</span>
              <span className="mt-1.5 text-sm text-[#71857e]">hoặc bấm để chọn một hay nhiều ảnh</span>
              <span className="mt-5 rounded-lg border border-[#d5dfd9] bg-white px-4 py-2 text-xs font-semibold shadow-sm">Chọn file HEIC</span>
            ...
            {!!items.length && <div className="mt-5">
              <div className="mb-3 flex items-center justify-between px-1"><div><h2 className="text-sm font-bold">Ảnh đã chọn ({items.length})</h2><p className="text-xs text-[#7b8e87]">Chất lượng JPG: {Math.round(quality * 100)}%</p></div><button onClick={clear} disabled={converting} className="flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-xs font-semibold text-[#a64c42] hover:bg-[#fff0ed]"><Trash2 size={14} /> Xóa tất cả</button></div>
